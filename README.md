...
NOTE: For a more behaved version of the bot that can still access memories across models, use `gpt_discord_bot.py` instead of `gpt_discord_bot_v2.py`. It does not have any reminder capabilities, but it can probably still play tick tack toe. You won't need to use the keras layer in that one.
...
* To DO: Cache training data for tic tac toe game. The Ai should get better as time goes on.
       ...
       That is then included in the training data for the GPT model.
       We could also try training another layer in-house to do that part, but I really want to see how far I can push openAi without fine-tuning.
```
